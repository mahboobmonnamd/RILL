# ADR 0028: Development surfaces are panes, not a second editor plane
...
- **Requires:** [ADR 0020](0020-session-graph-navigation-model.md) (pane slots),
  [ADR 0024](0024-non-terminal-panes-are-cold.md) (cold surfaces, browser out of
  process), [ADR 0026](0026-trust-secrets-and-automation-boundary.md) (trust,
  redaction)
- **Amends:** nothing.
- **Does not authorize:** authoring a text editor in this tree, an LSP client, a
  debug adapter client, agents (ADR 0030, ADR 0031), cloud indexing, sending
...
`hx` already run correctly in a leaf (F-102, ADR 0022 D4).
...
the working tree, on ADR 0024 D1's cold terms.
...
MUST use a declared per-editor invocation from config (ADR 0025 D1), resolved
...
(ADR 0026 D4) applies to anything the index hands to another surface.
...
- **Local notebooks (F-248):** runnable markdown is a **file the user owns**,
  not a hosted document and not an account feature (ADR 0026 D5). Running a
  fenced command follows ADR 0024 D3: visible command, explicit invocation,
...
object (ADR 0030 D2), not a private channel into a CLI agent's PTY.

A comment MUST NOT be injected as keystrokes into an agent's terminal by this
surface. Agent input goes through ADR 0031's adapter, with its permission
profile (ADR 0031 D5) applying. Nothing here ships before M3.
...
pane and MUST run in its out-of-process content process (ADR 0024 D2). They MUST
...
- F-244 ships nothing until M3 and ADR 0031.
...
- **Cloud/remote codebase indexing for better context.** Rejected: D5 and
  ADR 0026 D5. The row's own promise is that it stays local.
