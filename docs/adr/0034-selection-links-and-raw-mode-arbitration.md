# ADR 0034: Selection, hyperlinks and raw-mode arbitration
...
- **Requires:** [ADR 0009](0009-direct-to-display-echo.md),
  [ADR 0022](0022-terminal-fidelity-is-chip0.md) D1/D3 (Chip 0 owns parsing and
  arbitration), [ADR 0028](0028-development-surfaces-are-panes.md) D4 (editor
  resolver, argv), [ADR 0032](0032-blocks-are-a-cold-overlay.md),
  [ADR 0033](0033-input-editor-history-and-completion.md)
- **Amends:** nothing.
...
ADR 0022 D3 already set the one arbitration rule (Shift reclaims while mouse
...
The clipboard is a sink: redaction (ADR 0026 D4) applies to copy.
...
The target is **untrusted** (ADR 0026 D1). Therefore:
...
- The editor is invoked as an **argv vector** through ADR 0028 D4's resolver.
...
- A URL scheme MUST be on an allowlist (`http`, `https`, plus schemes the user
  added). A scheme RILL itself handles (deep links, ADR 0021 D5) gets that
...
the UI (ADR 0022 D3, F-084). This ADR adds no second rule and no heuristic.
...
(ADR 0033 D2) and MUST NOT alter the grid (D1).
...
tabs and panes (ADR 0020's tree).

Ctrl+Tab is the collision to respect: it MUST be resolvable through config
(ADR 0025 D5) and its load-time report MUST name it if a user binding would keep
...
(ADR 0026 D8), and MUST NOT write to any leaf (ADR 0021 D2).
...
NFR-KEY MUST hold with selection and highlighting active (ADR 0032 D3).

## Consequences

- [SPEC-MOUSE](../spec/SPEC-MOUSE.md) is the pointer, link and copy-mode
  contract.
- ADR 0028 D4's editor resolver is shared, not duplicated.
- ADR 0022 D3 remains the single arbitration statement.

## Rejected alternatives

- **Plain click opens paths and URLs.** Rejected: D2. Every accidental click
  becomes an action on attacker-chosen text.
- **Build the editor command as a shell string.** Rejected: D2, ADR 0028 D4.
