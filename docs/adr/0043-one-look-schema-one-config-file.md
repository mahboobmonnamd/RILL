...
- **Requires:** [ADR 0017](0017-ghostty-look-windowed-default.md) (look files,
  ...
  [ADR 0040](0040-terminal-fidelity-is-chip0.md) D7 (Ghostty import)
...
- **Does not authorize:** an account, a hosted settings service, a theme
  marketplace, plugins (ADR 0044 D3), `NSVisualEffectView` over Metal, per-cell
...
3. project-local trusted config (ADR 0044 D2) — only after trust
4. user config file
...
F-219. Project commands in the palette (ADR 0039 D2) are **declarations** in
config: a name, a command, a target. Running one MUST show the resolved command
before it runs.

A project-local action requires trust first (ADR 0044 D2). An action MUST NOT
...
Signature and notarization requirements are ADR 0044 D6; the pill MUST refuse to
