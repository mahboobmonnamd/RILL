# ADR 0029: Attention is an orchestration-plane queue
...
- **Requires:** [ADR 0001](0001-session-operating-system.md) (planes),
  ...
  [ADR 0020](0020-session-graph-navigation-model.md) (`NodeId`, projection),
  [ADR 0026](0026-trust-secrets-and-automation-boundary.md) (untrusted input,
  redaction)
- **Amends:** nothing.
- **Does not authorize:** agents or the `Task` object (ADR 0030), a cloud relay,
  push notifications through a server, an account, JSON on the warm path, a
  classifier that decides whether input is a prompt (ADR 0031 D9).
...
An entry has: a `NodeId` target (ADR 0020 D1), a state, an origin, a monotonic
...
independently. ADR 0024 D7's workspace status lane reads this queue; it does not
...
later an agent adapter (ADR 0031 D3).
...
every other sequence (ADR 0022 D1). The host MUST NOT scan the byte stream.

Their content is untrusted (ADR 0026 D1): it comes from any process, local or
...
(ADR 0026 D4), and MUST be attributed to the pane and host it came from.
...
BEL (F-103, ADR 0022 D8) raises an entry here rather than only making a sound.

Mutation `render_notify_body_as_markup` MUST turn T-ATT-UNTRUSTED red.

### D4 — CLI notify is the socket, with the socket's rules

F-118. `rill notify --title --body` goes over the daemon socket (ADR 0026 D7):
...
Remote notify (F-175, ADR 0023 D7) arrives on the same connection as its host's
...
- **Long-command complete (F-121):** a command that ran longer than a
  ...
  NOT guess (ADR 0022 D4's degrade rule).
...
(ADR 0026 D2). A hook MAY suppress, retag, or rewrite an entry's title and body.
...
- **Jump (F-113):** selecting an entry focuses the exact target — host,
  workspace, tab, pane, and later the section within a task (ADR 0030 D3). It
  navigates only; it MUST NOT write to the pane (ADR 0021 D2).
...
Agent-specific rollup semantics arrive with ADR 0030; the ordering above is
...
- ADR 0024 D7's `needs-attention` lane is a read of this queue.
- ADR 0022 D8's bell and ADR 0023 D7's remote relay are producers under D2.
- ADR 0030 adds agent producers without adding states.
