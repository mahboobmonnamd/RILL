# ADR 0042: Non-terminal panes are cold surfaces

- **Status:** Accepted — 2026-08-18
- **Amended by:** [ADR 0053](0053-runtime-domain-content-and-client-authority.md)
  D1, D7 and D11 for pane ownership, structured content and mobile clients.
- **Historical identifier:** merged as ADR 0024 in PR #278; renumbered to ADR
  0042 on 2026-08-21 to resolve the ADR 0038–0023 collision as one series.
  Renumbering changed no decision.
...
- **Requires:** [ADR 0009](0009-direct-to-display-echo.md),
  ...
  [ADR 0038](0038-session-graph-navigation-model.md) (pane slots, D2 stacks)
- **Amends:** nothing.
- **Does not authorize:** an editor or LSP (ADR 0046), agents, Blocks, Chip 1
...
built-in editor (F-194, deferred to ADR 0046), git worktree UI (F-195), mobile
...
ADR 0038 D2 already gave a pane slot a stack of surfaces. This ADR decides what
...
- open a URL scheme that RILL handles (deep links, ADR 0039 D5) without the same
  confirmation a foreign deep link gets,
- read the clipboard, or reach OSC 52 (ADR 0040 D3),
...
- The file explorer opens a file into a viewer or hands the path to an external
  editor (F-245, ADR 0046 D4). It MUST NOT edit in place.
...
- The diff viewer reads `git` output. Hunk revert (F-243) is ADR 0046 D3, and is
  not authorized here.
...
ports) attach to the `Workspace` node from ADR 0038 D1 and persist with it.
...
- Status lanes (`todo` / `working` / `needs-attention` / `done`) MUST be derived
  ...
  (ADR 0047 D1), not a second classifier. Manual override is allowed and MUST be
  marked as manual.
- Env (F-204) feeds `spawn_leaf` **before** spawn (ADR 0040 D5). It MUST NOT be
  written into a running shell.
- Advertised ports are **observed** from the child process, not declared by the
  user and assumed. An advertised port MUST NOT open a tunnel (ADR 0041 D7).
- Todos (F-203) are local files in the workspace, user-owned. Not a cloud
  object, not an agent's task list (that is ADR 0048 D5).
...
ADR 0041's protocol over a transport, and needs an identity story this tree does
not have. This ADR authorizes **only** the design constraint: any future
companion MUST speak the same attach protocol (ADR 0041 D1) and MUST NOT require
...
- [SPEC-SURFACES](../spec/SPEC-SURFACES.md) carries §1–§7 for these panes and
  §8–§11 for ADR 0046's development surfaces.
- ADR 0038's pane stack is the only place a surface lives; there is no floating
...
- **Editable file explorer in M2.** Rejected: D3. Editing is ADR 0046, and it
