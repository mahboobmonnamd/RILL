# ADR 0023: Remote is a second kernel, not a second display path
...
- **Requires:** [ADR 0001](0001-session-operating-system.md),
  ...
  [ADR 0020](0020-session-graph-navigation-model.md) (host indicator, F-001)
...
- **Does not authorize:** an account or control plane, cloud relay of PTY bytes,
  agents over SSH (ADR 0031), remote Blocks, shipping a bundled `ssh`, tunnels
...
Running tmux *inside* a leaf is supported and MUST keep working (ADR 0021 D6).
...
- **Notify relay (F-175):** remote attention events travel as orchestration on
  the same connection and land in the local queue (ADR 0029 D1). They carry the
...
- F-001's host indicator (ADR 0020 D6) becomes meaningful here and MUST show the
...
- **Relay through a hosted service for NAT traversal.** Rejected: D1. No third
  party on the PTY path, and no account (ADR 0026 D5).
