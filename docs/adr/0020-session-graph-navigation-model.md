# ADR 0020: Session graph is the navigation model
...
- **Does not authorize:** Blocks, agents, conversations, Chip 1 live, remote
  hosts (ADR 0023), non-terminal surfaces (ADR 0024), JSON or cells on the warm
...
ADR 0023; this ADR does not authorize a second kernel.
...
- **Hidden tabs stop reading the PTY to save memory.** Rejected: stalls the
  child, breaks NFR-DROP. Bounded memory is F-099 (ADR 0022 D6), a ring policy,
