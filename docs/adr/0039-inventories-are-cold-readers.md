# ADR 0039: Inventories, pickers and summon are cold readers

- **Status:** Accepted — 2026-08-18
- **Amended by:** [ADR 0053](0053-runtime-domain-content-and-client-authority.md)
  D2. Hidden management UI does not limit or delete the objects projected by
  cold readers.
- **Historical identifier:** merged as ADR 0021 in PR #278; renumbered to ADR
  0039 on 2026-08-21 to resolve a collision. Renumbering changed no decision.
...
- **Requires:** [ADR 0011](0011-session-graph.md),
  ...
  [ADR 0038](0038-session-graph-navigation-model.md) (container tree)
- **Amends:** nothing.
- **Does not authorize:** agents or an agent runtime (ADR 0048, ADR 0049),
  remote hosts (ADR 0041), plugins (ADR 0044), Blocks, Chip 1 live, a second
...
ADR 0038 put the container tree in the kernel. These rows are its readers. The
...
Reopen closed (F-016) restores a **layout template** (ADR 0038 D4), not a live
...
is no agent runtime until ADR 0048. A dashboard that shows fabricated agent rows
...
- add or modify a trusted project config (that is ADR 0044 D2),
...
declarative, or they are a plugin under ADR 0044 D3's out-of-process boundary.
...
entry hands the file to an agent surface once one exists (ADR 0048). Secret
redaction (ADR 0044 D4) applies to anything it renders.
...
- F-011 stays an empty inventory until ADR 0048 lands the `Task` object.
...
- **An in-process scripting runtime for sidebars.** Rejected: ADR 0044 D3
...
- **Scraping tmux to present it as native panes.** Rejected here and in
  ADR 0041 D6: a mirrored tmux is not a leaf the kernel owns.
