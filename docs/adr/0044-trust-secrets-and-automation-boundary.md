# ADR 0044: Trust, secrets, updates and the automation boundary
...
- **Requires:** [ADR 0001](0001-session-operating-system.md) (fail closed),
  ...
  [ADR 0039](0039-inventories-are-cold-readers.md) D5 (deep links),
  [ADR 0043](0043-one-look-schema-one-config-file.md) D3 (resolution order)
- **Amends:** nothing.
- **Does not authorize:** an account system, a hosted control plane, a plugin
  marketplace backend, in-process plugins, agents (ADR 0049), telemetry of user
...
Trusted config MAY contribute palette actions (ADR 0043 D6) and env. It MUST NOT
contribute keybindings that shadow control characters (ADR 0043 D5), MUST NOT
...
NOT stall the display link or take the window (same constraint as ADR 0042 D2).
...
Any future optional service (settings sync ADR 0043 D7, cloud agents ADR 0049
D8, permalinks ADR 0050 D7) is additive. First run MUST NOT ask for an account,
...
(ADR 0043 D8). Applying an update MUST NOT kill the daemon's children — the
...
- ADR 0039 D5 (deep links) and ADR 0042 D2 (browser) are instances of D1.
- ADR 0049's agent permission profiles inherit D1 and D3 rather than inventing a
  second grant model.
