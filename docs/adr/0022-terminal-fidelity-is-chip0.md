# ADR 0022: Terminal fidelity belongs to Chip 0
...
- **Does not authorize:** Chip 1 as the live chip, a second VT on the warm path,
  Blocks (ADR 0032), shell-integration-driven Block boundaries, replacing
...
Shift (F-084, ADR 0034 D3). There is no heuristic, no "if it looks like a
...
opt-in. Write MAY be allowed with the same trust prompt ADR 0026 D2 defines.
...
integration. That coupling is ADR 0032 D2's problem and is decided there.
...
(ADR 0025 D3) and the container node (ADR 0020 D1).
...
a line is unbounded. Hidden panes keep draining (ADR 0020 D2); memory is
...
leaf. BEL MUST also raise the attention path once that exists (ADR 0029 D3).
...
- ADR 0032 (Blocks) inherits D4: Blocks may not require shell integration.
