# ADR 0030: Task is the agent runtime object
...
- **Requires:** [ADR 0001](0001-session-operating-system.md),
  ...
  [ADR 0020](0020-session-graph-navigation-model.md),
...
- **Does not authorize:** a model provider, provider SDKs, agent detection
  (ADR 0031), Blocks (ADR 0032), cloud sync, an account, writing to a PTY from a
  task surface without ADR 0031's adapter, JSON on the warm path.
...
inside a chrome view model, for the same reason ADR 0020 D1 put containers in
...
(ADR 0031). A test that needs a network call is not evidence, because it cannot
...
Section content is untrusted (ADR 0026 D1): it may originate from a model, a
tool, or a remote process. It MUST NOT be rendered as executable markup, and
MUST be redacted at every persisting sink (ADR 0026 D4).
...
- An `approval` section blocks the task until the user approves, rejects, or
  ...
  is a permission-profile change and goes through ADR 0031 D5.
...
- Both raise attention immediately: `approval` and `needs_input` respectively
  (ADR 0029 D1). Neither may be rate-limited away (ADR 0029 D5).
...
- **Diff review (F-146):** per-file and per-hunk apply/reject shares
  ADR 0028 D3's implementation and its guards — exact hunk, staleness refusal,
...
It MUST NOT be conflated with the user's workspace todos (F-203, ADR 0024 D7),
...
- pass redaction at attach (ADR 0026 D4), because attaching is transmission,
- respect the index rules (ADR 0028 D5) — an attachment MUST NOT smuggle
  untracked or ignored files into context.

A URL attachment fetches nothing until the user confirms; page content is
untrusted (ADR 0024 D2).
...
It is the debugging surface for ADR 0031's detection. If explain cannot answer,
...
- ADR 0029's queue gains two producers (`approval`, `question`) and no new
  states.
- ADR 0031's live adapters must emit D3's sections; the replay adapter is the
