# ADR 0032: Blocks are a cold overlay; Chip 0 stays the warm path
...
- **Requires:** [ADR 0001](0001-session-operating-system.md),
  ...
  [ADR 0022](0022-terminal-fidelity-is-chip0.md) D4 (no shell-integration
...
When the ring has dropped a range (ADR 0022 D6), the Block MUST render as
...
ADR 0022 D4, correctness MUST NOT depend on them: with no marks, output is
...
- **Raw mode same PTY (F-083):** entering the alternate screen selects raw
  ...
  (ADR 0022 D1); Blocks read that, they do not track it.
- **Shift+mouse (F-084):** while mouse reporting is on, Shift selects in the UI.
  This is ADR 0022 D3's single explicit rule, stated once, no heuristic.
...
- Copy command / output / both (F-071) copies from the replayed byte range.
  Redaction (ADR 0026 D4) applies — the clipboard is a sink.
...
F-080. A cloud permalink is an optional layer (ADR 0026 D5). Local copy MUST
...
upload (ADR 0026 D4), and MUST be a per-Block explicit action. Nothing ships in
M6; the constraint is fixed here.

Attach-as-context (F-079) hands a Block's byte range to a task under
ADR 0030 D9's scoping and redaction rules.
...
ADR 0031 D4's single gated path — it MUST require the capability, MUST respect
...
- ADR 0022 D6's ring bound becomes user-visible through Block truncation.
...
- **Require shell integration for Blocks to work at all.** Rejected: ADR 0022
