# ADR 0033: Input editor, history and completion
...
- **Requires:** [ADR 0009](0009-direct-to-display-echo.md),
  [ADR 0022](0022-terminal-fidelity-is-chip0.md) D3 (input arbitration, IME),
  [ADR 0025](0025-one-look-schema-one-config-file.md) D5 (keybindings),
  [ADR 0026](0026-trust-secrets-and-automation-boundary.md) D4 (redaction),
  [ADR 0032](0032-blocks-are-a-cold-overlay.md) (Blocks overlay, raw mode)
- **Amends:** nothing.
- **Does not authorize:** an editor or LSP (ADR 0028 D1), a natural-language
  classifier (ADR 0031 D9), sending history off the machine, an account, input
...
Blocks off (ADR 0032 D4), the shell's own line editor is the input, and RILL
...
With Blocks off, keys go to the PTY (ADR 0032 D4) and every feature in this ADR
...
F-051, F-052, F-054 (with ADR 0034 D1), F-254.
...
behaviour. They MUST be overridable through config bindings (ADR 0025 D5).
...
ADR 0025 D5's load-time report catches a binding that would swallow it.
...
merged for search. It is a **local file** the user owns (ADR 0026 D5). It MUST
NOT be uploaded, synced by default, or shared.

Redaction (ADR 0026 D4) applies: history is a persisting sink. A command line
...
any pane set, and disabling it MUST be reachable by keyboard (ADR 0026 D8).
...
NFR-KEY MUST hold with the field active (ADR 0032 D3).

## Consequences

- [SPEC-INPUT](../spec/SPEC-INPUT.md) is the field contract.
- ADR 0032's Blocks mode gates every feature here.
- ADR 0034 owns selection and hyperlinks in the **output** region; this ADR owns
  the input field.
