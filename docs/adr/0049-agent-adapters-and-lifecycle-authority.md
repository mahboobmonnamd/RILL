# ADR 0049: Agent adapters, lifecycle authority and capability gates

- **Status:** Accepted — 2026-08-18
- **Historical identifier:** merged as ADR 0031 in PR #278; renumbered to ADR
  0049 on 2026-08-21 with its series. Renumbering changed no decision.
...
- **Requires:** [ADR 0011](0011-session-graph.md),
  [ADR 0040](0040-terminal-fidelity-is-chip0.md) D1 (Chip 0 owns parsing),
  [ADR 0044](0044-trust-secrets-and-automation-boundary.md),
  [ADR 0047](0047-attention-is-an-orchestration-queue.md),
  [ADR 0048](0048-task-is-the-agent-runtime-object.md) (sections, replay first)
- **Amends:** nothing.
...
ADR 0048 defined the `Task` and required the replay adapter to land first. This
...
Every adapter, whatever its transport, emits ADR 0048 D3's closed section set:

1. **Replay** (ADR 0048 D2) — the conformance reference. Ships first.
...
Screen manifests MUST be data (config, ADR 0043 D1), versioned and
...
add a parser to the byte stream (ADR 0040 D1), MUST NOT copy the grid per frame,
and MUST NOT sample faster than ADR 0039 D1's 2 Hz ceiling.
...
The CLI verbs live on the daemon socket under ADR 0044 D7's rules. There is no
`exec` verb.
...
model as ADR 0044 D3's plugin capabilities, not a second one.
...
same workspace, or via ADR 0048 D4's remembered choices.
...
another workspace. An MCP server is untrusted (ADR 0044 D1) and MUST NOT reach a
...
under ADR 0044 D4 at every sink.
...
(ADR 0042 D4), created with the path shown and confirmed.
...
functional (ADR 0044 D5): local history complete, local agents working, no
account.
...
- sync MUST apply redaction (ADR 0044 D4) before anything leaves,
- a trigger from outside (Slack, GitHub) is untrusted input (ADR 0044 D1) and
...
ADR 0042 D2's browser boundary. It MUST run in the out-of-process browser, MUST
...
agent fixture. No gate may require a live provider (ADR 0048 D2).

## Consequences

- [SPEC-AGENT](../spec/SPEC-AGENT.md) is the adapter, authority and permission
  contract.
- ADR 0048's `Task` gains live producers without new section types.
- ADR 0044 D3's capability model is reused, not duplicated.
...
- **Proxying model requests through a RILL service.** Rejected: D6, ADR 0044 D5.
