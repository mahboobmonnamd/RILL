...
fn assert_grids_match(a: &PodGrid, b: &PodGrid) {
    ...
    for r in 0..a.rows {
        for c in 0..a.cols {
            ...
            assert_eq!(
                (x.map(|v| v.codepoint), x.map(|v| v.fg), x.map(|v| v.bg), x.map(|v| v.attrs)),
                (y.map(|v| v.codepoint), y.map(|v| v.fg), y.map(|v| v.bg), y.map(|v| v.attrs)),
...
fn t_resync_reopen_over_a_live_shell_restores_the_screen() {
    ...
    // Resync is a cold path: exactly one, no matter how much typing follows.
    for _ in 0..100 {
    ...
    assert_eq!(
        daemon.resync_count(),
        1,
        "resync ran more than once — it is on the warm path (FR-RESYNC)"
    );
...
fn t_attach_second_attach_is_refused() {
    ...
    let mut daemon =
        Daemon::bind(&sock, "/bin/sh", &["-c", "exec sleep 30"], Winsize::default()).expect("bind");
